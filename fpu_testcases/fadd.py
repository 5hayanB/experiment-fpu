import random, os, struct, numpy as np
from functools import reduce
from bitstring import BitArray


cwd = os.path.dirname(__file__)
iterations = 500
random.seed(7)
fadd: list[list[str]] = []


def bin_to_float(b: str) -> float:
    bf = int(b, 2).to_bytes(4, 'big')
    return struct.unpack('>f', bf)[0]


def main() -> None:
    for i in range(iterations):
        rs: list[str] = [''.join([f'{random.randint(0, 1):01b}',
                                  f'{random.randint(1, 254):08b}',
                                  f'{random.randint(0, (2 ** 23) - 1):023b}'])
                         for _ in range(2)]

        # noinspection PyTypeChecker
        fadd.append(
            [rs[0], rs[1], BitArray(float=reduce(
                lambda a, b: bin_to_float(a) + bin_to_float(b),
                rs
            ), length=32).bin]
        )

    for e in fadd:
        rs1: float = bin_to_float(e[0])
        rs2: float = bin_to_float(e[1])
        rd: float = bin_to_float(e[2])

        if np.float32(rs1 + rs2) != rd:
            print('Mismatch at:\n'
                  'rs1 = {0}\n'
                  'rs2 = {1}\n'
                  'rs1 + rs2 = {2}\n'
                  'rd        = {3}'.format(
                rs1, rs2, np.float32(rs1 + rs2), rd
            ))
            return

    print('All cases verified')

    fadd_seq: list[str] = [f'("b{e[0]}", "b{e[1]}") -> "b{e[2]}"'
                           for e in fadd]


    testbench1: str = """package FPU.FloatALU

import chisel3._, chiseltest._, org.scalatest.freespec.AnyFreeSpec


class FALU_Test extends AnyFreeSpec with ChiselScalatestTester {
  "FALU" in {
    test(new FALU) {
      falu =>
        val testcases: Seq[((String, String), String)] = Seq(
          """

    testbench2: str = f',\n{" " * 10}'.join(fadd_seq)

    testbench3: str = """
        )

        for (i <- testcases) {
          falu.io.input(0).poke(i._1._1.U)
          falu.io.input(1).poke(i._1._2.U)
          falu.io.input(2).poke(0.U)
          falu.io.aluCtl.poke(1.U)
          falu.io.roundingMode.poke(0.U)
          falu.io.detectTininess.poke(1.U)

          falu.clock.step(1)

          falu.io.out.expect(i._2.U)
        }
    }
  }
}"""

    # Writing test file
    with open('{0}/FALU_Test.scala'.format(cwd), 'w', encoding='UTF-8') as f:
        f.write(testbench1 + testbench2 + testbench3)

    with open(f'{cwd}/FADD_testcases.txt', 'w', encoding='UTF-8') as f:
        for e in fadd:
            f.write(f'{bin_to_float(e[0])} + {bin_to_float(e[1])} = {bin_to_float(e[2])}\n')


if __name__ == '__main__':
    main()